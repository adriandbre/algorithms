function binarySearch(orderdArray, target) {
    let leftPointer = 0, rightPointer = orderdArray.length - 1;

    while (leftPointer <= rightPointer) {
        let middle = Math.floor((leftPointer + rightPointer) / 2)
        let posibleMatch = orderdArray[middle];

        if (target === posibleMatch) {
            return middle;
        } else if (posibleMatch < target) {
